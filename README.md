...
`optimizer.html`을 열면 검증된 영어 아이템 목록에서 수량을 선택하고, 모든 아이템을 6열 × 9행 보드에 배치해 유효한 Star 연결 수를 최대화할 수 있습니다. 기존 Shop Odds와 분리된 완전 오프라인 페이지이며 두 도구는 상단 내비게이션으로 이동합니다.
...
현재 optimizer 데이터 기준은 게임 `6.0.1`, 검증일 `2026-08-05`입니다. 현재 보드 아이템 940개 전체를 검색 목록에 포함하며 이미지·데이터는 모두 로컬입니다. 이 중 구조화된 footprint와 명시적인 Star 대상 조건을 확보한 889개는 계산할 수 있습니다. 고정 전체 보드 모델과 충돌하는 Bag 32개와 출처 문구가 불완전한 19개는 목록에는 표시하되 이유와 함께 선택을 차단합니다. 기존 15개 수동 검증 fixture는 생성 데이터보다 우선 적용됩니다. 지원 범위, 출처, 확인 결과는 [`DATA_NOTES.md`](DATA_NOTES.md)에 기록했습니다.
